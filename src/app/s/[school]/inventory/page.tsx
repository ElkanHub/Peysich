    ...
    <div className="max-w-2xl">
          ...
          <Tr key={i.id}>
            ...
            <Td>
              <div className="flex gap-1">
                <form action={adjustQty.bind(null, slug, i.id, 1)}><SubmitButton className="rounded border border-border px-2 py-0.5 text-xs">+1</SubmitButton></form>
                <form action={adjustQty.bind(null, slug, i.id, -1)}><SubmitButton className="rounded border border-border px-2 py-0.5 text-xs">−1</SubmitButton></form>
              </div>
            </Td>
          </Tr>
      ...
      <Card className="mt-5">
        <form action={addItem.bind(null, slug)} className="flex items-end gap-2">
          ...
          <SubmitButton className={btnCls}>Add item</SubmitButton>
        </form>
      </Card>
    </div>
  );
