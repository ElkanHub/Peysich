...
export default async function Settings({ params, searchParams }: {
  ...
  return (
    <div className="max-w-3xl space-y-6">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {tms.filter((t) => t.yearId === y.id).map((t) => (
                <form key={t.id} action={setCurrentTerm.bind(null, slug, t.id)}>
                  <SubmitButton className={t.isCurrent ? btnCls : btnGhostCls}>{t.name}</SubmitButton>
                </form>
              ))}
            </div>
          </div>
      ...
      {/* ── Academic structure: levels grouped preschool → JHS, classes inline ── */}
      <Card>
        <h2 className="font-semibold">Academic structure</h2>
        <p className="mt-0.5 text-[13px] text-muted-foreground">
          Levels grouped Preschool → Primary → Junior High, with each level&apos;s classes.
                      ...
                      <div key={l.id} className="rounded-lg border border-border p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          {myClasses.length === 0 ? (
                            ...
                            <span className="text-[11.5px] text-muted-foreground">{myClasses.length} class{myClasses.length > 1 ? "es" : ""}</span>
                          )}
                        </div>
                        <div className="mt-2 space-y-1.5">
                            ...
                            return (
                              <div key={c.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-muted/40 px-2.5 py-1.5">
                                <form action={renameClass.bind(null, slug, c.id)} className="flex items-center gap-1.5">
                                  ...
                                  <SubmitButton className="rounded border border-border px-2 py-1 text-[11px] hover:bg-muted">Rename</SubmitButton>
                                </form>
                                <span className="flex items-center gap-2 text-[12px] text-muted-foreground">
                                  ...
                                  <form action={setClassRoom.bind(null, slug, c.id)} className="flex items-center gap-1">
                                    ...
                                    <SubmitButton className="rounded border border-border px-1.5 py-0.5 text-[11px] hover:bg-muted">Set</SubmitButton>
                                  </form>
                                  {removable ? (
                                    <form action={deleteClass.bind(null, slug, c.id)}>
                                      <SubmitButton className="rounded border border-border px-1.5 py-0.5 text-[11px] text-danger hover:bg-muted">Remove</SubmitButton>
                                    </form>
                                  ) : (
                                    <span title={`${enrols} enrolment records protect this class's history`}
                                      className="cursor-help text-[11px] text-faint">protected</span>
                                  )}
                                </span>
                              </div>
                            );
                          ...
                          <form action={addClass.bind(null, slug)} className="flex items-center gap-1.5 pl-2.5">
                            ...
                            <SubmitButton className="rounded border border-border px-2 py-1 text-[11px] hover:bg-muted">+ Add class</SubmitButton>
                          </form>
                        </div>
                      </div>
                    );
      ...
      {/* ── Subjects: the curriculum catalogue with live usage ── */}
      <Card>
        <h2 className="font-semibold">Subjects</h2>
        <p className="mt-0.5 text-[13px] text-muted-foreground">
          Renaming updates score sheets, allocations, the timetable and teacher competencies together.
          Published report cards are immutable snapshots and always keep their history.
        </p>
              ...
              <li key={sub.id} className="py-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="flex items-center gap-2">
                    <span className="text-[12px] text-muted-foreground" data-nums="">
                ...
                {used && (
                  <details className="mt-1">
                    <summary className="cursor-pointer text-[12px] text-danger">Remove anyway…</summary>
                    <form action={deleteSubject.bind(null, slug, sub.id)}
                      className="mt-1.5 flex flex-wrap items-center gap-2 rounded-md bg-danger/5 px-3 py-2 text-[12.5px]">
                      ...
                      <SubmitButton className="rounded border border-danger/40 px-2 py-1 text-[11.5px] text-danger hover:bg-danger/10">
                        Delete subject
                      </SubmitButton>
                    </form>
                  </details>
      ...
      <Card>
        ...
        <p className="text-sm text-muted-foreground">
          Physical spaces — classrooms, labs, library, hall. Seat counts feed the enrolment
          screen&apos;s capacity display; the home room shows on each student&apos;s file.
        </p>
        {rms.length > 0 && (
            ...
            {rms.map((r) => (
              <li key={r.id} className="rounded-md border border-border px-3 py-2">
                ...
                <form action={deleteRoom.bind(null, slug, r.id)} className="mt-1 text-right">
                  <SubmitButton className="text-[11.5px] text-danger underline-offset-2 hover:underline">Remove room</SubmitButton>
                </form>
              </li>
            ))}
          </ul>
